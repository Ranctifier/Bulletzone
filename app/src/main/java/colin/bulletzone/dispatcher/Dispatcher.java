package colin.bulletzone.dispatcher;
...
import colin.bulletzone.AppContext;
import colin.bulletzone.R;
import colin.bulletzone.adapter.BulletZoneAdapter;
import colin.bulletzone.cell.cell_data.AbstractCellData;
import colin.bulletzone.cell.cell_graphic.AbstractCellGraphic;
import colin.bulletzone.cell_factory.SimpleCellDataFactory;
import colin.bulletzone.cell_factory.SimpleCellGraphicFactory;
import colin.bulletzone.game.ControlConstraints;
import colin.bulletzone.game.GameActivity;
import colin.bulletzone.rest.BulletZoneRestClient;
import colin.bulletzone.update.GridUpdateEvent;
import colin.bulletzone.update.ShakerListener;
...
public class Dispatcher {
    ...
    public void makeShaker(){
        ...
        if(null != shaker)
        { // avoids error on devices without an accelerometer
    ...
    /**
     * Sends the rest command for fire to the server.
     * Fire is called by the ShakerListener when receiving a shake.
     */
    ...
    public void fire(){
        ...
        if(clientTankAlive && ControlConstraints.canFire(bulletsOnGrid,
            ...
            try {
                ...
                if(rce.getStatusCode().value() != 404)
                { // error we don't know how to handle
                    throw rce;
                }
                else
                { // likely sent an http while tank dead
                    endError();
                }
            }
        }
    ...
    /**
     * Sends the turn clockwise command to the server.
     * turnClockwise is called by the activity_main.xml when the turnClockwise
     * button is pressed.
     */
    ...
    /**
     * Sends the turnCounterClockwise command to the server.
     * Called when the turnCounterClockwise button is pressed by the user.
     */
    @Background(serial = "turning")
     ...
     * Whenever this class receives the appropriate event from the
     * BusProvider, updateGrid is called with that event.
     */
    ...
    /**
     * Creates the data and graphical cells for the current grid on a
     * background thread. After the cells are created, updateGrid is called,
     * which updates the UI to reflect the new set of cells.
     * @param event
     */
    @Background
    ...
    /**
     * Used to stop the sessionCaretaker from collecting data and to add
     * that data to the replay database.
     */
    ...
    /**
     * Used to end the game when an http 404 error occurs during one of the
     * rest commands.
     * Note: Errors from join and leave not handled yet.
     */
