...
import colin.bulletzone.game.GameActivity_;
import colin.bulletzone.replay.ReplayActivity_;
   ...
   AppContext appContext;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        { // intent extra exists
            ...
            if(source.equals("GameActivity"))
            { // the source is GameActivity
                if(null != message)
                {
                    if(!message.equals("done"))
                    { // likely a http sent after client tank died
                        topMessage = message;
                    }
                }
            }
            else if(source.equals("ReplayActivity"))
            { // the source is ReplayActivity
                if(null != message)
                {
                    if(!message.equals("done"))
                    { // likely no replay was stored in DB
                        topMessage = message;
                    }
                }
            }
    ...
    protected void onResume() {
        ...
        if(null != topMessage)
        { // if message isn't null, use the textview
            gameMessage.setText(topMessage);
        }
        else
        { // otherwise, make the textview empty
            gameMessage.setText("");
        }
    }
