package colin.bulletzone.adapter;
...
public class BulletZoneAdapter extends BaseAdapter {
    ...
    /**
     * Sets the dimensions for the array of this adapter, assumes the array
     * must be re-instantiated and that the relevant layouts must be updated.
     * @param rows
     * @param columns
     */
     ...
     * Assumes that the gridFrame UI element has a layout width of 400dp and
     * a layout height of 400dp.
     * First obtains the scale factor from the pixel density of the display.
     * A conversion from dp to pixels is in the form of:
     * int pixels = (int) (dp * scale + 0.5f);
     * The conversion formula is from the android documentation.
     */
    private void updateLayouts(boolean inGame) {
        ...
        GridView grid;
        if(inGame)
        { // called from Dispatcher via GameActivity
            frame = (FrameLayout) inflater.inflate(R.layout.game_activity, null);
        }
        else
        { // called from ReplayActivity
     ...
     *
     * Obtains the AbstractCellGraphic using the specified index and removes
     * the passed view is either replaced or reused. If replaced, the view has
     * its contents set to the bitmap in the AbstractCellGraphic. The view is
     * then re-added to the viewGroup at the view's original index.
     * @param i
     * @param view
     * @param viewGroup
     * @return
