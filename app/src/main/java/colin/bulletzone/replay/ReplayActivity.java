package colin.bulletzone.replay;
...
import colin.bulletzone.MainActivity_;
...
public class ReplayActivity extends Activity {
    ...
    public void initReplay() {
        ...
        if(null != data)
        { // decode the replay when data exists
            decodeCursor(data);
            replayDB.close();
        ...
        { // show the back to main menu button and tell user data isn't there
    ...
    /**
     * Queries the replayDB to populate sessionData with information.
     * Returns true if the query was successful and data arrived,
    ...
    /**
     * Obtains and decodes the data in the sessionData cursor.
     * Data parsed into a start time, end time, and a string for gridState.
     * gridState is parsed to create a 2D array of AbstractGridData. That 2D
     * array and the times are used to add to an array of replayTriples.
     *
     ...
     * for the replay is determined. The array of replay triples is iterated
     * to populate the adapter with cell graphics. On the first iteration
     * after the adapter is populated, the grid ui is shown. Each iteration
     * updates the grid ui and then the next iteration is delayed by a call to
    ...
    public void runReplay(ReplayTriple[] replayArray) {
        ...
        { // iterate replay steps
            data = replayArray[i].grid;
            for(int j = 0; j < rows; j++)
            { // populate adapter with the triple's grid
                for(int k = 0; k < cols; k++)
                {
            ...
            if(i == 0)
            { // show the grid and then start updating it
                showGrid();
            }
            updateGrid();
